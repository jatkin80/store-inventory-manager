...
entryForm.addEventListener("submit", (event) => {
  ...
  const item = {
    ...
    dateAdded: format(parseISO(formData.get("date-added")), "D", {
...
function backstagePasses(item) {
  ...
  if (item.sellIn > 10 && newSellIn > 10) {
    ...
    item.quality = qualityCheck(
      item.quality + (item.sellIn - 10) + double(10 - newSellIn)
    );
    item.sellIn = newSellIn;
    return item;
  } else if (item.sellIn > 10 && newSellIn <= 5 && newSellIn > 0) {
    item.quality = qualityCheck(
      item.quality + (item.sellIn - 10) + 10 + triple(5 - newSellIn)
    );
    item.sellIn = newSellIn;
    return item;
  } else if (
    ...
    item.quality = qualityCheck(
      item.quality + double(item.sellIn - 5) + triple(5 - newSellIn)
    );
    item.sellIn = newSellIn;
    return item;
  } else return item;
