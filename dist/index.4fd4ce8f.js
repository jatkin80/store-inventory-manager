...
})({"gW2AJ":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
    ...
    "bNKaB": [
        function(require, module, exports) {
            ...
            function parseCategory(item) {
                if (item.name.includes("Aged Brie")) item.category = "Aged Brie";
                else if (item.name.includes("Sulfuras")) item.category = "Sulfuras";
                else if (item.name.includes("Backstage")) item.category = "Backstage passes";
                else if (item.name.includes("Conjured")) item.category = "Conjured";
                else item.category = "none";
                return item;
            }
            function elapsedDays(item) {
                return today - new Date(`${item.dateAdded}`);
            }
            function calculateQuality(item, time) {
                const time = elapsedDays(dateAdded, item);
                ...
                switch(item.category){
                    ...
                    case "Conjured":
                        sellIn -= time;
                        if (sellIn >= 0) quality -= time * 2;
                        else quality -= 2 * sellIn + 4 * (time - sellIn);
                        break;
                    case "Backstage passes":
                        sellIn -= time;
                        if (sellIn > 10) quality += time;
                        else if (sellIn <= 10 && sellIn > 5) {
                            if (sellIn > 10) quality += sellIn - 9 + 2 * (time - (sellIn - 10));
                            else quality += time * 2;
                        } else if (item.sell_in <= 5 && item.sell_in > 0) {
                            if (sellIn > 10) quality += sellIn - 10 - 3 + 3 * (time - (sellIn - 10));
                            else if (sellIn > 5) quality += sellIn + 1 + 3 * (time - (sellIn - 5));
                            else quality += time * 3;
                        } else quality = 0;
                        break;
                    default:
                        sellIn -= time;
                        if (sellIn >= 0) quality -= time;
                        else quality -= sellIn + 2 * (time - sellIn);
                }
                qualityCheck(item);
                return item;
            }
            function checkQuality(item) {
                if (item.category === "Sulfuras") return item.quality = 80;
                else if (item.category === "Aged Brie" && item.quality < 50) return item.quality;
                else if (item.category === "Backstage passes" && item.quality < 50) return item.quality;
                else if (item.quality > 50) return item.quality = 50;
                else if (item.quality <= 0) return item.quality = 0;
            ...
            function createList(item) {
                ...
                newRow.innerHTML = `
            <td>${item.name}</td>
            <td>${calculateQuality}</td>
            <td>${elapsedDays}</td>
    `;
                tableData.replaceChildren(newRow);
                return newRow;
            }
        },
        {
 ...
 */ var formatters = {
    ...
    "jKwHT",
    "bNKaB"
], "bNKaB", "parcelRequire5c5d");

},{}]},["gW2AJ","fUqB3"], "fUqB3", "parcelRequire5c5d")
